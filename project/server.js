...
process.on("uncaughtException", async (error) => {
    console.error(
        "FATAL UNCAUGHT EXCEPTION! Shutting down gracefully...",
        error,
    );
    ...
    if (dbManager && dbManager.db) {
        dbManager.db.close((err) => {
            if (err)
                console.error(
                    "Error closing DB on uncaught exception:",
                    err.message,
                );
        });
    }
...
process.on("unhandledRejection", async (reason, promise) => {
    console.error(
        "FATAL UNHANDLED REJECTION! Shutting down gracefully...",
        reason,
    );
    await sendTelegramLog(
    ...
    if (dbManager && dbManager.db) {
        dbManager.db.close((err) => {
            if (err)
                console.error(
                    "Error closing DB on unhandled rejection:",
                    err.message,
                );
        });
    }
...
if (!token) {
    console.error(
        "FATAL ERROR: TELEGRAM_BOT_TOKEN environment variable is not set!",
    );
}
...
if (LOGGER_BOT_TOKEN && LOG_CHAT_ID) {
    try {
        ...
        console.error(
            "❌ Failed to initialize Telegram logger bot:",
            e.message,
        );
        loggerBot = null;
    }
    ...
    console.warn(
        "⚠️ Telegram logger bot token or chat ID not configured. Telegram logging disabled.",
    );
}
...
async function sendTelegramLog(message) {
    if (loggerBot && LOG_CHAT_ID) {
        try {
            await loggerBot.sendMessage(LOG_CHAT_ID, `[BOT LOG] ${message}`, {
                parse_mode: "Markdown",
            });
        } catch (error) {
...
async function sendMainMenu(
    chatId,
    messageText = "Please choose an option from the menu below:",
) {
...
async function fetchStudentAttendance(classId, phoneNumber) {
    const apiUrl =
        "https://api.teachusapp.com/teachus/attendance/student_attendance_list";
    ...
    if (!jwtToken) {
        console.error(
            "FATAL ERROR: TEACHUS_API_JWT_TOKEN environment variable is not set!",
        );
        return {
            success: false,
            message: "Internal server error: API authorization token missing.",
        };
    }
    ...
    try {
        console.log(
            `Sending API request for class_id: ${classId}, phone: ${phoneNumber}, subject_id: ${subjectId}`,
        );
        ...
        if (!parsedData || !Array.isArray(parsedData.student_list)) {
            const warnMessage =
                "API response does not contain a 'student_list' array as expected.";
            console.warn(warnMessage);
            console.warn(
                "Full unexpected API response:",
                JSON.stringify(parsedData, null, 2),
            );
            sendTelegramLog(
                `⚠️ API Response Warning: ${warnMessage} Full response: \`\`\`json\n${JSON.stringify(parsedData)}\n\`\`\``,
            );
            return {
                success: false,
                message:
                    "Attendance data is not available in the expected format for this course. (Missing 'student_list')",
            };
        }
        ...
        if (student) {
            ...
            return {
                success: false,
                message: `No data found for your phone number (${phoneNumber}) in the selected course.`,
            };
        }
    } catch (error) {
        console.error("❌ Error fetching student attendance:", error.message);
        let errorMessage =
            "Could not connect to the attendance service. Please check your internet connection or try again later.";
        if (error.response) {
            ...
            sendTelegramLog(
                `❌ API Error (Status ${error.response.status}) for class_id \`${classId}\`, phone \`${phoneNumber}\`. Data: \`\`\`json\n${JSON.stringify(error.response.data)}\n\`\`\``,
            );
        } else {
            sendTelegramLog(
                `❌ API Connection Error for class_id \`${classId}\`, phone \`${phoneNumber}\`. Error: \`${error.message}\``,
            );
        }
...
if (bot) {
    ...
    bot.onText(/^\/start(?: (.+))?$/, async (msg, match) => {
        ...
        console.log(
            `📞 Message from Telegram chat ID ${chatId}: "/start" (Referrer: ${referrerId})`,
        );
        ...
        if (referrerId && isNewUser) {
            ...
            const referralResult = await referralManager.recordReferral(
                referrerId,
                chatId,
            );
            if (referralResult.success) {
                await bot.sendMessage(
                    chatId,
                    `🎉 Welcome! ${referralResult.message}`,
                );
                sendTelegramLog(
                    `🔗 User \`${chatId}\` joined via referral from \`${referrerId}\`. Reward applied: ${referralResult.message}`,
                );
            } else {
                await bot.sendMessage(
                    chatId,
                    `👋 Welcome! There was an issue with your referral: ${referralResult.message}`,
                );
                sendTelegramLog(
                    `⚠️ User \`${chatId}\` joined via referral from \`${referrerId}\`, but referral failed: ${referralResult.message}`,
                );
            }
            ...
            await bot.sendMessage(
                chatId,
                `👋 Welcome back! It looks like you've already started the bot before.`,
            );
            sendTelegramLog(
                `ℹ️ User \`${chatId}\` (existing) started via referral from \`${referrerId}\`. No new referral recorded.`,
            );
        } else {
            ...
            sendTelegramLog(
                `🔄 User \`${chatId}\` started/restarted bot (no referral).`,
            );
        }
        ...
        try {
            switch (data) {
                case "check_attendance":
                    ...
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    sendTelegramLog(
                        `✅ User \`${chatId}\` chose 'Check Attendance'.`,
                    );
                    break;
                ...
                case "my_status":
                    ...
                    sendTelegramLog(
                        `📊 User \`${chatId}\` requested 'My Status'.`,
                    );
                    await sendMainMenu(
                        chatId,
                        "Here's your status. What's next?",
                    ); // Return to main menu
                    break;
                ...
                case "plans":
                    ...
                    await sendMainMenu(
                        chatId,
                        "Explore our plans! What's next?",
                    ); // Return to main menu after showing plans
                    break;
                ...
                case "invite_earn":
                    ...
                    sendTelegramLog(
                        `🤝 User \`${chatId}\` requested invite link.`,
                    );
                ...
                case "talk_with_us":
                    ...
                    botResponseText =
                        "Please type your query or feedback now. I will forward it to the admin. Type '/cancel' to go back.";
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    sendTelegramLog(
                        `📝 User \`${chatId}\` chose 'Talk with Us'.`,
                    );
                    break;

                default:
                    botResponseText =
                        "I didn't understand that. Please choose from the main menu.";
                    await sendMainMenu(chatId, botResponseText);
                    sendTelegramLog(
                        `❓ User \`${chatId}\` sent unknown callback_data: \`${data}\`.`,
                    );
                    break;
            }
        } catch (error) {
            ...
            await bot.sendMessage(
                chatId,
                "Oops! Something went wrong. Please try again or type `/start`.",
            );
    ...
    bot.onText(/^\/reset_referral (.+)$/, async (msg, match) => {
        ...
        sendTelegramLog(
            `🔑 Admin \`${chatId}\` attempting to reset referral for user \`${referredUserIdToReset}\`.`,
        );
        const success = await referralManager.resetReferral(
            referredUserIdToReset,
        );
        if (success) {
            await bot.sendMessage(
                chatId,
                `✅ Referral for user \`${referredUserIdToReset}\` has been reset (deleted).`,
            );
            sendTelegramLog(
                `✅ Admin \`${chatId}\` *RESET* referral for user \`${referredUserIdToReset}\`.`,
            );
        } else {
            await bot.sendMessage(
                chatId,
                `❌ No referral found or failed to reset for user \`${referredUserIdToReset}\`.`,
            );
        }
    });
    ...
    bot.onText(/^\/revoke_referral (.+)$/, async (msg, match) => {
        ...
        sendTelegramLog(
            `🔑 Admin \`${chatId}\` attempting to revoke reward for user \`${referredUserIdToRevoke}\`.`,
        );
        const result = await referralManager.revokeReferralReward(
            referredUserIdToRevoke,
        );
        if (result.success) {
            await bot.sendMessage(chatId, `✅ ${result.message}`);
            sendTelegramLog(
                `✅ Admin \`${chatId}\` *REVOKED* reward for user \`${referredUserIdToRevoke}\`.`,
            );
        } else {
            await bot.sendMessage(chatId, `❌ ${result.message}`);
        }
    ...
    bot.onText(/^\/config_reward (\d+) (.+)$/, async (msg, match) => {
        ...
        if (!["free_checks", "plan_discount"].includes(type)) {
            // Example types
            await bot.sendMessage(
                chatId,
                "❌ Invalid reward type. Supported types: `free_checks`.",
            );
            return;
        }

        await referralManager.setSetting(
        ...
        await bot.sendMessage(
            chatId,
            `✅ Referral reward configured: ${amount} of type '${type}'.`,
        );
        sendTelegramLog(
            `🔑 Admin \`${chatId}\` configured referral reward to: ${amount} ${type}.`,
        );
    });
    ...
    bot.on("message", async (msg) => {
        ...
        if (
            ...
            userMessage.startsWith("/reset_referral") ||
            userMessage.startsWith("/revoke_referral") ||
            userMessage.startsWith("/config_reward")
        ) {
            return;
        }

        console.log(
            `📞 Text message from Telegram chat ID ${chatId}: "${userMessage}"`,
        );
        ...
        try {
            ...
            if (
                ...
                sendTelegramLog(
                    `↩️ User \`${chatId}\` typed 'back' to main menu.`,
                );
            ...
            // Buy command still expects a text input
                ...
                if (planDetails) {
                    ...
                    console.log(
                        `💬 Bot Response: User ${chatId} initiated enrollment in ${planKey}.`,
                    );
                    sendTelegramLog(
                        `💳 User \`${chatId}\` attempting to buy plan: *${planDetails.name}*.`,
                    );
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    ...
                    botResponseText =
                        "❌ Invalid program. Please use `/plans` to see available options.";
                    console.log(
                        `💬 Bot Response: User ${chatId} entered invalid plan key.`,
                    );
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                ...
                if (await dbManager.activatePlan(chatId, planKey)) {
                    ...
                    sendTelegramLog(
                        `💰 User \`${chatId}\` *ENROLLED* in plan: *${dbManager.getPlanDetails(planKey).name}*.`,
                    );
                } else {
                    botResponseText = `❌ Payment failed or program activation error. Please try again or contact support.`;
                    sendTelegramLog(
                        `⛔ User \`${chatId}\` payment *FAILED* for plan: *${dbManager.getPlanDetails(planKey).name}*.`,
                    );
                }
                ...
                if (
                    await dbManager.isHiddenFeatureUnlocked(
                        chatId,
                        "exclusive_strategies_tips",
                    )
                ) {
                    botResponseText =
                        "📈 Here are exclusive strategies and tips to boost your attendance: [Link to resource / Mock Data for Tips]";
                    sendTelegramLog(
                        `🎁 User \`${chatId}\` accessed *Exclusive Strategies & Tips*.`,
                    );
                } else if (
                    await dbManager.isHiddenFeatureUnlocked(
                        chatId,
                    ...
                    botResponseText =
                        "📞 You have priority consultation! Please contact us to schedule your 1-on-1 session via email at support@example.com (Mock Support Info).";
                    sendTelegramLog(
                        `🎁 User \`${chatId}\` accessed *Priority Consultation*.`,
                    );
                } else if (
                    await dbManager.isHiddenFeatureUnlocked(
                        chatId,
                        "personalized_coaching_analytics",
                    )
                ) {
                    botResponseText =
                        "📊 Access your Personalized Analytics Dashboard and coaching schedule here: [Mock URL for Analytics / Coaching]";
                    sendTelegramLog(
                        `🎁 User \`${chatId}\` accessed *Personalized Coaching & Analytics*.`,
                    );
                } else {
                    botResponseText =
                        "🔐 This is a premium attendance boosting feature. Please enroll in a program to unlock it!";
                    sendTelegramLog(
                        `🚫 User \`${chatId}\` attempted to access hidden feature without plan.`,
                    );
                }
                await bot.sendMessage(chatId, botResponseText, {
                    parse_mode: "Markdown",
            ...
            // --- Query/Feedback Feature ---
                ...
                if (userMessage === "/cancel") {
                    ...
                    botResponseText =
                        "Query submission cancelled. Returning to the main menu.";
                    sendTelegramLog(
                        `✖️ User \`${chatId}\` cancelled query submission.`,
                    );
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    ...
                    botResponseText =
                        "Thank you! Your query has been successfully submitted. We will get back to you if needed. Returning to the main menu.";
            ...
            // Handle pagination and course selection only when in the correct state
                ...
                const totalCoursesCount =
                    await dbManager.getTotalCoursesCount();
                const totalCoursePages = Math.ceil(
                    totalCoursesCount / COURSES_PER_PAGE,
                );

                if (userMessage === "f") {
                    if (session.coursePage < totalCoursePages - 1) {
                        session.coursePage++;
                        botResponseText = await getCourseBatch(
                            session.coursePage,
                        );
                        console.log(
                            `💬 Bot Response: User navigated forward to page ${session.coursePage + 1}.`,
                        );
                    } else {
                        botResponseText =
                            "🚫 You're already at the **last page**. Use 'b' to go back or select a course number.";
                    }
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    ...
                    if (session.coursePage > 0) {
                        ...
                        botResponseText =
                            "🚫 You're already at the **first page**. Use 'f' to go forward or select a course number.";
                    }
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                    ...
                    const selectedCourse =
                        await dbManager.getCourseById(selectedNumber);
                    ...
                    if (selectedCourse) {
                        ...
                        sendTelegramLog(
                            `✅ User \`${chatId}\` selected course: *${selectedCourse.name}* (\`${selectedCourse.id}\`).`,
                        );
                        await bot.sendMessage(chatId, botResponseText, {
                            parse_mode: "Markdown",
                        });
                        ...
                        botResponseText =
                            "❌ Invalid course number. Please use 'f', 'b', or enter a valid number from the list.\n\n_Type 'back' to return to the main menu._";
                        sendTelegramLog(
                            `❌ User \`${chatId}\` entered invalid course number: \`${userMessage}\`.`,
                        );
                        await bot.sendMessage(chatId, botResponseText, {
                            parse_mode: "Markdown",
                        });
                    ...
                    botResponseText =
                        "❌ Invalid input. To navigate, reply 'f' for next, 'b' for back, or enter a number to choose a course.\n\n_Type 'back' to return to the main menu._";
                    sendTelegramLog(
                        `❌ User \`${chatId}\` invalid input in course selection state: \`${userMessage}\`.`,
                    );
                    await bot.sendMessage(chatId, botResponseText, {
                        parse_mode: "Markdown",
                    });
                ...
                if (/^\d{10}$/.test(submittedPhoneNumber)) {
                    ...
                    if (!courseId) {
                        botResponseText =
                            "It seems I lost track of your course selection. Please type 'back' to return to the main menu.";
                        clearSession(chatId);
                        sendTelegramLog(
                            `⚠️ Missing course ID for \`${chatId}\` in AWAITING_PHONE state. Session cleared.`,
                        );
                        await bot.sendMessage(chatId, botResponseText, {
                            parse_mode: "Markdown",
                        });
                        ...
                        const usageStatus =
                            await dbManager.canPerformCheck(chatId);
                        if (!usageStatus.allowed) {
                            botResponseText = `🚫 ${await dbManager.getUsageStatus(chatId)}\n\nTo continue, please purchase a plan using \`/plans\` or choose another option from the main menu.`;
                            sendTelegramLog(
                                `🚫 User \`${chatId}\` hit free limit for attendance check or plan expired.`,
                            );
                            ...
                            sendTelegramLog(
                                `📞 User \`${chatId}\` checking attendance for course *${courseName}* (\`${courseId}\`) with phone: \`${submittedPhoneNumber}\`.`,
                            );

                            const apiResult = await fetchStudentAttendance(
                                courseId,
                            ...
                            if (apiResult.success) {
                                ...
                                const student = apiResult.data;
                                botResponseText =
                                    `✅ Attendance Report for *${student.name}* in *${courseName}*:\n` +
                    ...
                    botResponseText =
                        "❌ Invalid phone number format. Please enter a valid **10-digit mobile number**, like `9876543210`.\n\n_Type 'back' to return to the main menu._";
                    sendTelegramLog(
                        `❌ User \`${chatId}\` entered invalid phone format: \`${userMessage}\`.`,
                    );
                ...
                sendTelegramLog(
                    `❓ User \`${chatId}\` sent unexpected text in MAIN_MENU state: \`${userMessage}\`.`,
                );
            } else {
                // Catch-all for unexpected states
                botResponseText =
                    "I'm not sure what you mean. Let's get you back to the main menu.";
                clearSession(chatId);
                sendTelegramLog(
                    `⚠️ User \`${chatId}\` in unexpected state: \`${session.state}\` with message \`${userMessage}\`. Session cleared, prompting main menu.`,
                );
                await bot.sendMessage(chatId, botResponseText, {
                    parse_mode: "Markdown",
                });
            ...
            sendTelegramLog(
                `🚨 *Unhandled Error in Bot Logic:*\nChatID: \`${chatId}\`\nMessage: \`${userMessage}\`\nError: \`${error.message}\`\nStack: \`${error.stack ? error.stack.substring(0, 1000) + "..." : "N/A"}\``,
            );
            await bot.sendMessage(
                chatId,
                "Oops! Something went wrong on our end. Please try again by typing `/start`.",
            );
            clearSession(chatId);
    ...
    console.error(
        "❌ Telegram bot not initialized due to missing token. Bot functionality disabled.",
    );
}
